  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-toss-black">타임테이블 관리</h1>
          <p className="text-sm text-toss-gray mt-1">수련회 전체 일정을 일자별로 관리합니다.</p>
        </div>
      ...
      <div className="flex p-1 bg-toss-lightGray rounded-2xl w-fit">
        {(["DAY1", "DAY2", "DAY3"] as Day[]).map((day) => (
          ...
          >
            {day} <span className="ml-1 opacity-50 font-medium">{dayInfo[day]}</span>
          </button>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-8 py-6 border-b border-toss-border flex justify-between items-center">
              <h2 className="text-xl font-black text-toss-black">새 일정 추가</h2>
              <button onClick={() => setIsAdding(false)} className="p-2 hover:bg-toss-lightGray rounded-full transition-colors">
                <Plus size={24} className="rotate-45 text-toss-gray" />
              </button>
            </div>
            ...
            <form className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-toss-gray px-1">일자</label>
                  <select 
                    className="w-full px-4 py-3 rounded-xl border border-toss-border focus:border-toss-blue outline-none transition-all appearance-none bg-white font-bold text-sm"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-toss-gray px-1">시간</label>
                  <div className="relative">
                    <Clock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-toss-gray" />
                    <input 
                      type="time" 
                      className="w-full pl-11 pr-4 py-3 rounded-xl border border-toss-border focus:border-toss-blue outline-none transition-all font-bold text-sm"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-toss-gray px-1">프로그램 타이틀</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-toss-gray px-1">장소</label>
                <div className="relative">
                  <MapPin size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-toss-gray" />
                  <input 
                    type="text" 
                    placeholder="예: 그랜드볼룸, 식당 등"
                    className="w-full pl-11 pr-4 py-3 rounded-xl border border-toss-border focus:border-toss-blue outline-none transition-all text-sm font-medium"
                  />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-toss-gray px-1">상세 내용</label>
                <textarea 
                  rows={3}
                  placeholder="프로그램에 대한 설명을 입력하세요"
                  className="w-full px-4 py-3 rounded-xl border border-toss-border focus:border-toss-blue outline-none transition-all resize-none text-sm leading-relaxed"
                ></textarea>
              </div>
      ...
      <div className="bg-white rounded-3xl border border-toss-border shadow-sm overflow-hidden">
        <div className="p-6 border-b border-toss-border flex justify-between items-center">
          <h3 className="font-bold text-toss-black flex items-center gap-2">
            <CalendarIcon size={18} className="text-toss-blue" />
            {activeDay} 상세 일정
          </h3>
          <span className="text-xs font-bold text-toss-gray bg-toss-lightGray px-3 py-1 rounded-full">
        ...
        <div className="divide-y divide-toss-border">
          {filteredSchedules.length > 0 ? (
            filteredSchedules.map((item) => (
              <div key={item.id} className="p-6 flex items-start gap-6 hover:bg-toss-lightGray/30 transition-colors group">
                <div className="pt-1 cursor-move text-toss-border group-hover:text-toss-gray transition-colors">
                  <GripVertical size={20} />
                </div>
                
                <div className="w-24 shrink-0 pt-0.5">
                  <div className="flex items-center gap-1.5 text-toss-blue font-black text-lg italic">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-[17px] font-black text-toss-black mb-1">{item.title}</h4>
                  <p className="text-sm text-toss-gray leading-relaxed mb-3">{item.description}</p>
                  <div className="flex items-center gap-1.5 text-xs font-bold text-toss-gray bg-toss-lightGray/50 w-fit px-2.5 py-1 rounded-lg border border-toss-border/50">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button className="p-2.5 text-toss-gray hover:text-toss-blue hover:bg-toss-blue/5 rounded-xl transition-all border border-transparent hover:border-toss-blue/10">
                    <Edit2 size={18} />
                  </button>
                  <button className="p-2.5 text-toss-gray hover:text-red-500 hover:bg-red-50 rounded-xl transition-all border border-transparent hover:border-red-100">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
